...
区間 \([4.7,4.75]\) において \(Q(a_c)=0\) を満たす唯一の臨界軸比 \(a_c\) が存在し、認証区間 \((4.72438,\,4.72439)\) に属し、
\[
...
\caption{局所分岐図。中心枝 \(r=0\) は全軸比で存在する。破線は認証済み局所係数による主項
\(r_*(a)^2=c(a_c-a)\) を示す。\(c=6Q'(a_c)/H_4(a_c)\) は認証包絡 \([0.3415,0.4516]\) に属する。
\(c\approx0.39474123\) は参考点推定値であり認証値ではない。実測枝または大域的数値追跡ではない。}
...
群作用に由来し、法方向 Hessian が非退化であれば \(C_a\) は Morse–Bott 臨界多様体となる。 半径方向固有値は負である。
軸方向固有値は、\(Q_z(a_c)>0\)、\(r(a)\to0\) および Hessian の連続性により、十分近い \(a<a_c\) の停留円上でも正である。
...
Moszyńska \cite{Moszynska}、Herburt–Moszyńska–Peradzyński \cite{HMP}、Sakata \cite{Sakata}、O'Hara \cite{OHara} らの中心理論は、凸体や領域に付随する
...
\begin{itemize}
\item 全12認証物件を \path{SHA256SUMS.txt} に固定し、\texttt{sha256sum -c} は 12/12 OK である。\par
...
\item 0F1 導関数鎖監査：\path{prolate_h_chain_symbolic_audit.py}。\(h(x)=\arccos(x)^2\) の1--4階導関数との symbolic difference は全て0。
\item Qz 生核監査：\path{prolate_Qz_symbolic_audit.py}。抽象関数 \(h\) を保持した生の重み付き核を直接二階微分する。
\item Qz 区間認証：\path{prolate_Qz_arb_certificate.py} / \path{prolate_Qz_arb_certificate.json}。本文下界 gate は \(0.0885587746621582\)。
