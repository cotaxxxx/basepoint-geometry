...
Here a label-preserving local trivialization means a homeomorphism $\Phi:\mathcal S\cap(I\times U)\to I\times F$ with $\operatorname{pr}_I\circ\Phi=\pi$ such that corresponding connected components in the fibers carry the same component-dimension, normal-index, nullity, and isotropy labels.
\end{definition}
...
\]
\begin{lemma}[Real analyticity near the center]\label{lem:analyticity}
For $a$ in a neighborhood of $[4.7,4.75]$ and $p$ sufficiently close to the center, $E_a(p)$ is real analytic in $(a,p)$.
\end{lemma}
Indeed $h(x)=\arccos^2x$ has a removable analytic endpoint at $x=1$,
\[
h(x)=2(1-x)+\frac13(1-x)^2+O((1-x)^3),
\]
while convexity and compactness of the parameter neighborhood keep the radial--normal cosine uniformly separated from $-1$. The geometric factors in the boundary parametrization are analytic, and on the compact angular domain their derivatives admit uniform bounds. Hence the local power series may be integrated term by term, giving analytic dependence on $(a,p)$. In particular the even radial expansion above, the factorization $\partial_rE_a=rG(a,r^2)$, and the Hadamard factorization used below are justified by this lemma.

\subsection{Canonical coefficient kernels}
...
\[
\ell=s^2+a^2c_\theta^2,\qquad w=(a^2s^2+c_\theta^2)^{1/2},\qquad C=\frac{a}{w\sqrt\ell},
\]
and write $h_j=h^{(j)}(C)$ for $h(x)=\arccos^2x$. The transverse quadratic kernel is
\[
B_2=\frac{C}{\ell^2}\!\left\{Ch_2(\ell-1)^2s^2+h_1\big[(2\ell^2-4\ell+3)s^2-2\ell\big]\right\},
\qquad Q(a)=\frac12\int_0^{\pi/2}sB_2\,d\theta.
\]
For the fourth derivative, define
\begin{align*}
A_4=\frac{C}{\ell^4}\{&C^3h_4(\ell-1)^4\\
&+C^2h_3(4\ell^4-24\ell^3+54\ell^2-52\ell+18)\\
&+Ch_2(-24\ell^3+108\ell^2-168\ell+87)\\
&+h_1(36\ell^2-120\ell+105)\},\\
A_2=-\frac{6C}{\ell^3}\{&C^2h_3(\ell-1)^2+Ch_2(4\ell^2-12\ell+9)\\
&+h_1(2\ell^2-12\ell+15)\},\\
A_0=\frac{3C}{\ell^2}(Ch_2+3h_1),\qquad
B_4=\frac34A_4s^4+A_2s^2+2A_0,
\end{align*}
so that $H_4(a)=\frac12\int_0^{\pi/2}sB_4\,d\theta$. For the axial certificate put $v=ac_\theta$, $w_z=c_\theta/a$, and $d=v/\ell-w_z$. The audited raw second derivative is
\[
F_z''(0)=C^2h_2d^2+Ch_1\!\left(-\frac1\ell+\frac{3v^2}{\ell^2}-\frac{4w_zv}{\ell}+2w_z^2\right),
\qquad Q_z(a)=\int_0^{\pi/2}sF_z''(0)\,d\theta.
\]
These are exactly the kernels used by the SHA-pinned symbolic audits and Arb/Acb certificates described in Section~\ref{sec:repro}.
...
\[
\operatorname{Crit}(E_a)=\{0\}\sqcup C_a\quad(a<a_c\ \text{sufficiently close}),
\]
...
\section{Reproducibility and proof objects}\label{sec:repro}
