#!/usr/bin/env python3
"""Source-bound rigorous adapter candidate v2 for Item 3 sweep v9.

STATUS: VALIDATION CANDIDATE / NOT PRODUCTION APPROVED.

V2 preserves the validated V1 mean-value path and adds the public rigorous endpoint/box
interface `evaluate_g` required by the S1/S2 root-existence checks.  It contains no
runner/checker state machine, checkpoint logic, GitHub API calls, or verdict promotion.
"""
...
ADAPTER_ID = "ITEM3_SWEEP_V9_MEAN_VALUE_ADAPTER_CANDIDATE_V2"
...
def _combine_arb_associations(
    *, expression_id: str, direct_value: arb, factored_value: arb
) -> QuotientEvidence:
    ...
    if direct.finite and factored.finite:
        ...
        return QuotientEvidence(
            expression_id, direct, factored, "INTERSECTION", _canonical_interval(final_value)
        )
    if direct.finite:
    ...
    return QuotientEvidence(
        expression_id, direct, factored, "NONFINITE", CanonicalInterval.nonfinite()
    )
...
def _quotient_gr(F: arb, F_r: arb, R: arb) -> QuotientEvidence:
    ...
    return _combine_arb_associations(
        ...
        direct_value=(F_r / R) - (F / R2),
        factored_value=((F_r * R) - F) / R2,
    )
...
def _quotient_grr(F: arb, F_r: arb, F_rr: arb, R: arb) -> QuotientEvidence:
    ...
    return _combine_arb_associations(
        ...
        direct_value=((F_rr / R) - ((2 * F_r) / R2)) + ((2 * F) / R3),
        factored_value=(((F_rr * R2) - ((2 * F_r) * R)) + (2 * F)) / R3,
    )


def _quotient_grlambda(F_lambda: arb, F_rlambda: arb, R: arb) -> QuotientEvidence:
    R2 = R * R
    return _combine_arb_associations(
        expression_id="ITEM3_V9_GRLAMBDA_DUAL_ASSOC_V1",
        direct_value=(F_rlambda / R) - (F_lambda / R2),
        factored_value=((F_rlambda * R) - F_lambda) / R2,
    )
...
class V9MeanValueAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        module_name: str = "item3_v9_pinned_candidate_kernel_v2",
    ) -> None:
    ...
    @staticmethod
    def _validate_dps(dps: int) -> None:
    ...
    def evaluate_g(
        ...
        *,
        ...
        """Rigorous enclosure of G=F/r on a positive r point/interval and lambda box."""
        self._validate_coordinates(r_cell, lambda_box)
        self._validate_dps(dps)
        old_dps = ctx.dps
        try:
            ctx.dps = dps
    ...
    def evaluate_mean_value(
        ...
        *,
        ...
        lambda_score = (
            None if not g_rlambda.final.finite else radius(lambda_box) * g_rlambda.final.absmax()
        )
