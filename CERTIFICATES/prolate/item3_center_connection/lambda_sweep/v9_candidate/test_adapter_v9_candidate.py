...
def main() -> int:
    ...
    record(checks, "adapter_id_exact", adapter.adapter_id == a.ADAPTER_ID)
    record(checks, "kernel_pre_hash_exact", adapter.kernel_identity.pre_import_sha256 == KERNEL_SHA)
    record(checks, "kernel_post_hash_exact", adapter.kernel_identity.post_import_sha256 == KERNEL_SHA)
    record(
        checks,
        "kernel_origin_exact",
        Path(adapter.kernel_identity.module_origin) == (REPO_ROOT / KERNEL_REL).resolve(),
    )
    ...
    try:
        ...
        record(checks, "wrong_kernel_hash_rejected", True)
    else:
        record(checks, "wrong_kernel_hash_rejected", False)
    ...
    try:
        ...
        record(checks, "path_escape_rejected", True)
    else:
        record(checks, "path_escape_rejected", False)
    ...
    record(checks, "dual_overlap_intersection", inter.association_class == "INTERSECTION" and inter.final.finite)

    direct_only = a._combine_arb_associations(
        expression_id="TEST", direct_value=arb(1), factored_value=arb.nan()
    )
    record(checks, "dual_direct_only", direct_only.association_class == "DIRECT_ONLY" and direct_only.final.finite)

    factored_only = a._combine_arb_associations(
        expression_id="TEST", direct_value=arb.nan(), factored_value=arb(1)
    )
    record(checks, "dual_factored_only", factored_only.association_class == "FACTORED_ONLY" and factored_only.final.finite)

    neither = a._combine_arb_associations(
        expression_id="TEST", direct_value=arb.nan(), factored_value=arb.nan()
    )
    record(checks, "dual_both_nonfinite", neither.association_class == "NONFINITE" and not neither.final.finite)
    ...
    try:
        ...
        record(checks, "dual_disjoint_fatal", True)
    else:
        record(checks, "dual_disjoint_fatal", False)

    r_cell = (Fraction(1, 64), Fraction(129, 8192))
    lambda_box = (Fraction(123731943, 26214400), Fraction(118, 25))

    rb = a._interval_to_arb(r_cell)
    record(checks, "r_input_contains_lo", encloses(rb, a._fraction_to_arb(r_cell[0])))
    record(checks, "r_input_contains_hi", encloses(rb, a._fraction_to_arb(r_cell[1])))
    lb = a._interval_to_arb(lambda_box)
    record(checks, "lambda_input_contains_lo", encloses(lb, a._fraction_to_arb(lambda_box[0])))
    record(checks, "lambda_input_contains_hi", encloses(lb, a._fraction_to_arb(lambda_box[1])))

    old_dps = ctx.dps
    evidence = adapter.evaluate_mean_value(r_cell=r_cell, lambda_box=lambda_box, dps=50)
    record(checks, "ctx_dps_restored", ctx.dps == old_dps)
    record(checks, "seven_kernel_calls", evidence.kernel_calls == 7)
    record(checks, "canonical_r_center", evidence.r0 == (r_cell[0] + r_cell[1]) / 2)
    record(checks, "canonical_lambda_center", evidence.lambda0 == (lambda_box[0] + lambda_box[1]) / 2)
    record(checks, "g_r_center_finite", evidence.g_r_center.final.finite)
    record(checks, "g_rr_box_finite", evidence.g_rr_box.final.finite)
    record(checks, "g_rlambda_box_finite", evidence.g_rlambda_box.final.finite)
    record(checks, "r_score_finite", evidence.r_score is not None)
    record(checks, "lambda_score_finite", evidence.lambda_score is not None)
    record(checks, "known_left_cell_strict_NEG", evidence.strict_negative)
    ...
    details["r_cell"] = [str(r_cell[0]), str(r_cell[1])]
    ...
    details["adapter_source_sha256"] = hashlib.sha256(
        (HERE / "adapter_v9_candidate.py").read_bytes()
    ).hexdigest()
