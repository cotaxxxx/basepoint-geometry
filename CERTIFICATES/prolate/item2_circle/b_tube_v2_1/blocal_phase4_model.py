...
def validate_config(config: dict[str, Any]) -> None:
    exact_keys(config, {
        ...
        "budgets", "canonicalizer_id", "adapter", "arb_to_dyadic_adapter_sha256",
        "outputs", "terminal_state_before_run",
    }, "config")
    ...
    need(isinstance(config["implementation"]["sources_sha2556"], dict)
    ...
    need(config["arb_to_dyadic_adapter_sha256"] == config["adapter"]["source_sha2556"],
         "adapter source SHA identity")
...
def machine_conclusion(
    selected: tuple[int, Fraction, Fraction, dict[str, Any]] | None,
) -> dict[str, Any]:
    complete = selected is not None
    return {
        "binding_to_final_lambda_start": complete,
        "coverage_claim": complete,
        "lambda_start": rational_json(selected[1]) if selected else None,
        "real_analytic": False,
        "state": "BLOCAL_CERTIFIED" if complete else INCOMPLETE,
        "unique_non_degenerate_root_for_every_lambda_in": RANGE if complete else None,
    }
