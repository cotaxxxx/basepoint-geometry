...
EXACT_LAMBDA_TRANSPORT_PATH = HERE / "exact_lambda_transport.py"
...
def _fraction_interval_arb(lo: Fraction, hi: Fraction, arb_type):
    if hi < lo:
        raise CalibrationError("exact lambda transport: reversed lambda interval")
...
def _transport_evidence(model: Any, lo: Fraction, hi: Fraction) -> tuple[DyadicInterval, dict[str, Any]]:
    _exact_lambda_domain(lo, hi)
    if model.NORMALIZATION_BITS != 192:
        raise CalibrationError("exact lambda transport: frozen rounding-bit mismatch")
    lambda_plus = model.LAMBDA_PLUS
    s_lo = lo - lambda_plus
    s_hi = hi - lambda_plus
    rounded_lo = model.floor_dyadic(s_lo, model.NORMALIZATION_BITS)
    rounded_hi = model.ceil_dyadic(s_hi, model.NORMALIZATION_BITS)
    try:
        s_iv = DyadicInterval(
        ...
        raise CalibrationError("exact lambda transport: frozen rounding was not dyadic") from exc
    lower_enlargement = s_lo - rounded_lo
    upper_enlargement = rounded_hi - s_hi
    unit = Fraction(1, 1 << model.NORMALIZATION_BITS)
    if not (
    ...
    detail = {
        ...
        "lower_rounding_enlargement": Rational.from_fraction(lower_enlargement).to_json(),
        "rounding_bits": model.NORMALIZATION_BITS,
        ...
        "upper_rounding_enlargement": Rational.from_fraction(upper_enlargement).to_json(),
    }
    return s_iv, detail


def _install_runtime_enclose_f_guard(route: Any) -> None:
...
class ExactLambdaRoutedEvaluator(RoutedEvaluator):
    """Routed evaluator whose result-bearing API requires exact rational lambda provenance."""

    def __init__(self, interior_kernel: Any, arb_type: Any, config: dict[str, Any]):
        super().__init__(interior_kernel, arb_type, config)
        source_path = _assert_repo_regular_file(EXACT_LAMBDA_TRANSPORT_PATH, REPO_ROOT)
        expected = config.get("routed_evaluator_exact_lambda_transport_sha256")
        if not isinstance(expected, str) or sha256_hex(source_path.read_bytes()) != expected:
            raise CalibrationError("exact lambda transport: source pin mismatch")
        route = self.modules["blocal_v22_boundary"]
        _install_runtime_enclose_f_guard(route)
    ...
    def _boundary_exact(
        ...
        if proof.get("s_interval") != expected_s_json:
            raise CalibrationError("exact lambda transport: frozen proof s interval mismatch")
    ...
    def _evaluate_exact(
        ...
        *,
        ...
        if force_route is not None and force_route not in {
            ...
            raise CalibrationError("exact lambda transport: invalid forced backend route")
        ...
        if selected == ROUTED_INTERIOR_ROUTE_ID:
            ...
            _, ri, rd, ru = self._boundary_exact(
                quantity, right, lambda_lo, lambda_hi
            )
    ...
    def F_exact_arb(
        ...
        value, _, _ = self._evaluate_exact(
            "F", self._r_input(r), lambda_lo, lambda_hi, tol, depth, limit
        )
        return value
...
def _kernel_F_with_arb_type(
    kernel, arb_type, r_arg, lambda_lo: Fraction, lambda_hi: Fraction, *, tol, depth, limit
):
    ...
    *,
    ...
    for _ in range(iterations):
        ...
        residual = arb_ball_to_exact_interval(
            _kernel_F_with_arb_type(
                kernel, arb_type, point, lam, lam, tol=tol, depth=depth, limit=limit
            )
        )
        slope = arb_ball_to_exact_interval(
            _kernel_Fr_with_arb_type(
                kernel, arb_type, point, lam, lam, tol=tol, depth=depth, limit=limit
            )
        )
...
def install_exact_lambda_call_sites() -> dict[str, str]:
    ...
    targets = (
        (numeric_module, "_newton_predictor", "calibration_numeric", exact_newton_predictor),
        (candidate_module, "_newton_predictor", "calibration_numeric", exact_newton_predictor),
        (candidate_module, "_evaluate_krawczyk", "calibration_candidate", exact_evaluate_krawczyk),
        (anchor_module, "_newton_predictor", "calibration_numeric", exact_newton_predictor),
        (anchor_module, "_evaluate_point", "a0b_start_anchor", exact_evaluate_point),
    )
    for module, name, original_module, replacement in targets:
        ...
        if current is None or getattr(current, "__module__", None) != original_module:
            raise CalibrationError(
                f"exact lambda transport: unexpected patch target {module.__name__}.{name}"
            )
