...
def run(config_path: Path, output_directory: Path) -> dict[str, Any]:
    ...
    model.append_record(records, previous, {
        ...
        "records_chain_tip_sha256": chain_tip_before_summary,
        "terminal_state": model.COMPLETE if selected else model.INCOMPLETE,
    })

    conclusion = model.machine_conclusion(
        selected, totals, len(records), chain_tip_before_summary)
    certificate = {
        ...
        "budgets": config["budgets"],
        "chain_genesis": model.chain_genesis(config_hash),
        "chain_tip": chain_tip_before_summary,
