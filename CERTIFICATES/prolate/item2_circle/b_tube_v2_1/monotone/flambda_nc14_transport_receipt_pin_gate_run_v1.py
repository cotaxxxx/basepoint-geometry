#!/usr/bin/env python3
"""Dedicated deterministic NC14 transport-receipt-pin gate runner.

This is F_LAMBDA_CONTRACT_V1.1 preexecution infrastructure.

Scope:
- execute the V1.7 gate-unit harness;
- require the real-checker NC14 direct gate to pass;
...
HARNESS = HERE / "flambda_gate_unit_harness_v1_7.py"

EXPECTED_CODE = "FAIL_TRANSPORT_RECEIPT_PIN"
VERDICT = "NC14_EXACT_SUBCODE_PASS_NOT_PROMOTED"

REQUIRED_STDOUT_LINES = (
    "NC14_DIRECT_GATE=PASS",
...
def main() -> int:
    ...
    if head_post != head_pre:
        stop("HEAD changed during NC14 gate")
    if git("status", "--porcelain"):
        stop("SOURCE_TREE_POST dirty")

    receipt = {
        "schema": "flambda-nc14-transport-receipt-pin-gate-run-v1",
