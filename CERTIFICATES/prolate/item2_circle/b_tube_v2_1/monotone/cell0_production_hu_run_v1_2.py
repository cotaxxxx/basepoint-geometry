...
This runner deliberately reuses the released producer bytes unchanged.  It creates
an isolated detached worktree at the released producer execution head, executes
that producer afresh on the released policy/input, and attests the new execution
as PRODUCTION_CANDIDATE.  The historical positive-control receipt is never read
or copied.  Because policy, producer, input, and toolchain are identical, the new
raw receipt is required to be bit-identical to the released positive-control
result; any mismatch is fail-closed and must be investigated.
"""
...
def git_show_bytes(repo: Path, ref: str, relpath: str) -> bytes:
    return subprocess.check_output(["git", "-C", str(repo), "show", f"{ref}:{relpath}"])


def require_release_pins(repo: Path) -> None:
    ...
    try:
        ...
        fail("RELEASE_SHA_NOT_ANCESTOR")

    expected = {
        REL_POLICY: POLICY_SHA256,
        REL_PRODUCER: PRODUCER_SHA256,
        REL_CHECKER: CHECKER_SHA256,
    }
    for relpath, expected_sha in expected.items():
        observed = sha256_bytes(git_show_bytes(repo, RELEASE_TAG, relpath))
        if observed != expected_sha:
            fail("RELEASE_FILE_SHA_MISMATCH:" + relpath)
        current = repo / relpath
        if not current.is_file() or sha256_file(current) != expected_sha:
            fail("CURRENT_RELEASE_FILE_SHA_MISMATCH:" + relpath)
    ...
    if pins.get("independent_checker_sha256") != CHECKER_SHA256:
        fail("CONTRACT_CHECKER_PIN")
...
def main() -> int:
    ...
    raw_result_out = ns.raw_result.expanduser().resolve()
    raw_log_out = ns.raw_log.expanduser().resolve()
    out_json = ns.out_json.expanduser().resolve()

    if git(repo, "status", "--porcelain"):
        fail("SOURCE_TREE_PRE_DIRTY")
    execution_head = git(repo, "rev-parse", "HEAD")
    require_release_pins(repo)
    ...
    raw_result_out.parent.mkdir(parents=True, exist_ok=True)
    raw_log_out.parent.mkdir(parents=True, exist_ok=True)
    out_json.parent.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory(prefix="cell0-hu-v12-production-") as td:
        td_path = Path(td)
        worktree = td_path / "producer-worktree"
        fresh_raw = td_path / "fresh_raw.json"
        worktree_added = False
        try:
            subprocess.check_call(
                ["git", "-C", str(repo), "worktree", "add", "--detach", str(worktree), PRODUCER_EXECUTION_HEAD],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            )
            worktree_added = True
            producer = worktree / REL_PRODUCER
            policy = worktree / REL_POLICY
            if sha256_file(producer) != PRODUCER_SHA256:
                fail("DETACHED_PRODUCER_SHA")
            if sha256_file(policy) != POLICY_SHA256:
                fail("DETACHED_POLICY_SHA")
            if git(worktree, "status", "--porcelain"):
                fail("DETACHED_WORKTREE_PRE_DIRTY")
            ...
            raw_log_out.write_text(proc.stdout)
            if not fresh_raw.is_file():
                fail("FRESH_RAW_RESULT_MISSING")
            shutil.copyfile(fresh_raw, raw_result_out)
            if proc.returncode != 0:
                fail("RELEASED_PRODUCER_NONZERO")
            if git(worktree, "status", "--porcelain"):
                fail("DETACHED_WORKTREE_POST_DIRTY")
        finally:
            if worktree_added:
                subprocess.run(
    ...
    raw_sha = sha256_file(raw_result_out)
    bit_identical = raw_sha == POSITIVE_CONTROL_RESULT_SHA256
    if not bit_identical:
        fail("BIT_IDENTITY_MISMATCH")

    raw = json.loads(raw_result_out.read_text())
    if raw.get("contract_id") != "PRODUCTION_HU_DOMAIN_CONTRACT_V1_2":
        fail("RAW_CONTRACT_ID")
    if raw.get("policy_sha256") != POLICY_SHA256:
        fail("RAW_POLICY_SHA")
    if raw.get("runner_sha256") != PRODUCER_SHA256:
        fail("RAW_PRODUCER_SHA")
    if raw.get("execution_head") != PRODUCER_EXECUTION_HEAD:
        fail("RAW_EXECUTION_HEAD")
    if raw.get("verdict") != "POSITIVE_CONTROL_PASS":
        fail("RAW_RELEASED_PRODUCER_VERDICT")

    post_head = git(repo, "rev-parse", "HEAD")
    post_clean = not bool(git(repo, "status", "--porcelain"))
    head_unchanged = post_head == execution_head
    ...
    receipt = {
        "schema": "production-hu-domain-v1.2-cell0-production-attestation-v1",
        ...
        "released_checker_sha256": CHECKER_SHA256,
        "release_sha": RELEASE_SHA,
        "release_tag": RELEASE_TAG,
        "producer_execution_head": PRODUCER_EXECUTION_HEAD,
        "execution_head": execution_head,
        "runner_sha256": sha256_file(repo / REL_RUNNER),
        "raw_result_path": str(raw_result_out),
        "raw_result_sha256": raw_sha,
        "raw_log_path": str(raw_log_out),
        "raw_log_sha256": sha256_file(raw_log_out),
        "bit_identity_reference_sha256": POSITIVE_CONTROL_RESULT_SHA256,
        "bit_identical_to_positive_control": bit_identical,
        "raw_receipt_evidence_class": raw.get("evidence_class"),
        "raw_receipt_verdict": raw.get("verdict"),
    ...
    print("POSITIVE_CONTROL_RECEIPT_REUSED=FALSE")
    print("FRESH_REEXECUTION=TRUE")
    print("POLICY_SHA256=" + POLICY_SHA256)
    print("PRODUCER_SHA256=" + PRODUCER_SHA256)
    print("CHECKER_SHA256=" + CHECKER_SHA256)
    print("RAW_RESULT_SHA256=" + raw_sha)
    print("BIT_IDENTICAL_TO_POSITIVE_CONTROL=TRUE")
    print("CHECKER_STATUS=PENDING")
    print("SOURCE_TREE_PRE=CLEAN")
    print("SOURCE_TREE_POST=CLEAN")
    print("HEAD_UNCHANGED_DURING_RUN=TRUE")
    print("VERDICT=PRODUCTION_REPLAY_COMPLETE_CHECKER_PENDING")
    print("PRODUCTION_ATTESTATION=" + str(out_json))
    print("PRODUCTION_ATTESTATION_SHA256=" + sha256_file(out_json))
    return 0
