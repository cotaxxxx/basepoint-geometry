#!/usr/bin/env python3
"""Dedicated deterministic NC08 ordinary-formula-runtime-ID gate runner.

This is F_LAMBDA_CONTRACT_V1.1 preexecution infrastructure.

Scope:
- execute the V1.8 gate-unit harness;
- require the real-checker NC08 direct gate to pass;
...
VERDICT = "NC08_EXACT_SUBCODE_PASS_NOT_PROMOTED"

REQUIRED_STDOUT_LINES = (
    "NC08_DIRECT_GATE=PASS",
...
def main() -> int:
    ...
    if head_post != head_pre:
        stop("HEAD changed during NC08 gate")
    if git("status", "--porcelain"):
        stop("SOURCE_TREE_POST dirty")

    receipt = {
        "schema": "flambda-nc08-anchor-side-rhi-gate-run-v1",
