#!/usr/bin/env python3
"""Dedicated deterministic NC02a ordinary-formula-runtime-ID gate runner.

This is F_LAMBDA_CONTRACT_V1.1 preexecution infrastructure.

Scope:
- execute the V1.8 gate-unit harness;
- require the real-checker NC02a direct gate to pass;
...
HARNESS = HERE / "flambda_gate_unit_harness_v1_9.py"

EXPECTED_CODE = "FAIL_ORDINARY_FORMULA_RUNTIME_ID"
VERDICT = "NC02a_EXACT_SUBCODE_PASS_NOT_PROMOTED"

REQUIRED_STDOUT_LINES = (
    "NC02a_DIRECT_GATE=PASS",
...
def main() -> int:
    ...
    if head_post != head_pre:
        stop("HEAD changed during NC02a gate")
    if git("status", "--porcelain"):
        stop("SOURCE_TREE_POST dirty")

    receipt = {
        "schema": "flambda-nc02a-ordinary-formula-runtime-id-gate-run-v1",
