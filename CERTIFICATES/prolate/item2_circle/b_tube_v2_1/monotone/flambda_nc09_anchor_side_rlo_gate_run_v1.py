#!/usr/bin/env python3
"""Dedicated deterministic NC09 ordinary-formula-runtime-ID gate runner.

This is F_LAMBDA_CONTRACT_V1.1 preexecution infrastructure.

Scope:
- execute the V1.8 gate-unit harness;
- require the real-checker NC09 direct gate to pass;
...
HARNESS = HERE / "flambda_gate_unit_harness_v1_10.py"

EXPECTED_CODE = "FAIL_ANCHOR_SIDE_DERIVATION"
VERDICT = "NC09_EXACT_SUBCODE_PASS_NOT_PROMOTED"

REQUIRED_STDOUT_LINES = (
    "NC09_DIRECT_GATE=PASS",
...
def main() -> int:
    ...
    if head_post != head_pre:
        stop("HEAD changed during NC09 gate")
    if git("status", "--porcelain"):
        stop("SOURCE_TREE_POST dirty")

    receipt = {
        "schema": "flambda-nc09-anchor-side-rlo-gate-run-v1",
        "contract": "F_LAMBDA_CONTRACT_V1.1",
        "control_id": "NC09",
        "method": "SYNTHETIC_PREDICATE_REAL_CHECKER_NEED",
    ...
    print("CONTROL_ID=NC09")
    print("METHOD=SYNTHETIC_PREDICATE_REAL_CHECKER_NEED")
