#!/usr/bin/env python3
"""Independent verifier for an internal monotone JOIN overlap receipt."""
...
def promotion_gate(obj: dict, cell: str, kind: str, geometry_sha: str) -> None:
    ...
    expected = f"{cell}_{kind}_COMPONENT_ONLY"
    if effect.get("authorized_component") != expected:
...
def main() -> int:
    ...
    receipt_path = ns.receipt.resolve()
    rec = load(receipt_path)
    if rec.get("schema") != SCHEMA or rec.get("contract") != CONTRACT:
        fail("RECEIPT_SCHEMA")
    if rec.get("component") != COMPONENT:
        fail("RECEIPT_COMPONENT")
    if rec.get("proof_mode") != "C1_HU_OVERLAP_COMPOSITION":
        fail("PROOF_MODE")
    if rec.get("binding_use_authorized") is not False:
        fail("SOURCE_RECEIPT_SELF_AUTHORIZED")
    if rec.get("legacy_point_krawczyk_reexecuted") is not False:
        fail("LEGACY_KRAWCZYK_FLAG")
    if rec.get("legacy_adaptive_join_contract_claimed") is not False:
        fail("LEGACY_CONTRACT_FLAG")

    cells = rec.get("cells", {})
    left_id, right_id = cells.get("left"), cells.get("right")
    if not isinstance(left_id, str) or not isinstance(right_id, str):
        fail("CELL_IDS")
    left_index = int(left_id.removeprefix("CELL"))
    right_index = int(right_id.removeprefix("CELL"))
    if left_id != f"CELL{left_index}" or right_id != f"CELL{right_index}":
        fail("CELL_ID_FORMAT")
    ...
    pins = rec.get("pins", {})
    for key, name in rels.items():
        if pins.get(key + "_sha256") != sha(mon / name):
    ...
    if lg.get("candidate_inputs", {}).get("cell_index") != left_index:
        fail("LEFT_CELL_INDEX")
    if rg.get("candidate_inputs", {}).get("cell_index") != right_index:
    ...
    if rec.get("geometry") != expected_geometry:
        fail("GEOMETRY_RECONSTRUCTION")

    logical = rec.get("logical_composition", {})
    required_true = (
        "left_section_hu_positive",
        "right_section_hu_positive",
        "overlap_nonempty_open_interval",
        "f_r_nonzero_on_both_sections",
        "c1_available_on_both_tube_rectangles",
        "conditional_branch_identity_bridge",
        "internal_join_ready_for_monotone_assembly",
    )
    if any(logical.get(key) is not True for key in required_true):
        fail("LOGICAL_COMPOSITION")
    if rec.get("verdict") != (
        f"{left_id}_{right_id}_INTERNAL_JOIN_C1_HU_OVERLAP_PASS_READY_FOR_JUDGE_PROMOTION"
    ):
        fail("VERDICT")

    print("CHECKER_VERDICT=PASS")
    print("JOIN=" + left_id + "->" + right_id)
    print("INTERSECTION_WIDTH=" + expected_geometry["intersection_width"])
    print("SOURCE_RECEIPT_SHA256=" + sha(receipt_path))
    print("CHECKER_SHA256=" + sha(mon / rels["checker"]))
    return 0
