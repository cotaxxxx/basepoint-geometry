#!/usr/bin/env python3
"""Dedicated deterministic NC23 checker-DPS contract gate runner.

This is F_LAMBDA_CONTRACT_V1.1 preexecution infrastructure.

Scope:
- execute the V1.5 gate-unit harness;
- require the real-checker NC23 direct gate to pass;
...
HARNESS = HERE / "flambda_gate_unit_harness_v1_5.py"

EXPECTED_CODE = "FAIL_CHECKER_DPS_CONTRACT"
VERDICT = "NC23_EXACT_SUBCODE_PASS_NOT_PROMOTED"

REQUIRED_STDOUT_LINES = (
    "NC23_DIRECT_GATE=PASS",
...
def main() -> int:
    ...
    if head_post != head_pre:
        stop("HEAD changed during NC23 gate")
    if git("status", "--porcelain"):
        stop("SOURCE_TREE_POST dirty")

    receipt = {
        "schema": "flambda-nc23-checker-dps-contract-gate-run-v1",
