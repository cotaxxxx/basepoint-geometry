#!/usr/bin/env python3
"""Dedicated deterministic NC27 kernel-manifest-pin gate runner.

This is F_LAMBDA_CONTRACT_V1.1 preexecution infrastructure.

Scope:
- execute the V1.8 gate-unit harness;
- require the exact-checker-predicate NC27 direct gate to pass;
...
HARNESS = HERE / "flambda_gate_unit_harness_v1_8.py"

EXPECTED_CODE = "FAIL_KERNEL_MANIFEST_PIN"
VERDICT = "NC27_EXACT_SUBCODE_PASS_NOT_PROMOTED"

REQUIRED_STDOUT_LINES = (
    "NC27_DIRECT_GATE=PASS",
...
def main() -> int:
    ...
    if head_post != head_pre:
        stop("HEAD changed during NC27 gate")
    if git("status", "--porcelain"):
        stop("SOURCE_TREE_POST dirty")

    receipt = {
        "schema": "flambda-nc27-kernel-manifest-pin-gate-run-v1",
