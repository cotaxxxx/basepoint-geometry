#!/usr/bin/env python3
"""Dedicated deterministic NC28d signature-receipt-link gate runner.

This is F_LAMBDA_CONTRACT_V1.1 preexecution infrastructure.

Scope:
- execute the V1.11 gate-unit harness;
- require the real-checker NC28d direct gate to pass;
...
- no canonical tracked transport-artifact mutation claim;
...
EXPECTED_CODE = "FAIL_SIGNATURE_RECEIPT_LINK"
VERDICT = "NC28d_EXACT_SUBCODE_PASS_NOT_PROMOTED"

REQUIRED_STDOUT_LINES = (
    "NC28d_DIRECT_GATE=PASS",
...
def main() -> int:
    ...
    if head_post != head_pre:
        stop("HEAD changed during NC28d gate")
    if git("status", "--porcelain"):
        stop("SOURCE_TREE_POST dirty")

    receipt = {
        "schema": "flambda-nc28d-signature-receipt-link-gate-run-v1",
