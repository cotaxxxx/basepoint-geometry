#!/usr/bin/env python3
"""Finalize fresh cell-0 production H_U evidence using the released checker unchanged.

The released checker bytes are invoked directly.  Only after its PASS does this
script expose the narrow MONOTONE_TUBE interface: all terminal lower bounds are
positive, the exact terminal union equals the parent rectangle, and the certified
cover margin is strictly positive.
"""
...
def require_release_files(repo: Path) -> None:
...
def main() -> int:
    ...
    if raw_sha != POSITIVE_CONTROL_RESULT_SHA256:
        fail("RAW_BIT_IDENTITY_MISMATCH")
    ...
    if proc.returncode != 0:
        fail("RELEASED_CHECKER_NONZERO")
    if "INDEPENDENT_CHECKER_VERDICT=PASS" not in proc.stdout:
        fail("RELEASED_CHECKER_VERDICT_MISSING")
    ...
    receipt = {
        "schema": "production-hu-domain-v1.2-cell0-production-receipt-v1",
        "contract_id": "PRODUCTION_HU_DOMAIN_CONTRACT_V1_2",
        "evidence_class": "PRODUCTION_CANDIDATE",
        "binding_use_authorized": True,
        ...
        "bit_identical_to_positive_control": True,
    ...
    print("MONOTONE_NARROW_INTERFACE_AUTHORIZED=TRUE")
    print("SOURCE_TREE_PRE=CLEAN")
    print("SOURCE_TREE_POST=CLEAN")
    print("HEAD_UNCHANGED_DURING_RUN=TRUE")
    print("VERDICT=CELL0_PRODUCTION_HU_CHECKER_PASS")
