#!/usr/bin/env python3
"""Cell-0 start-join union H_U certificate V2 for MONOTONE_TUBE_V1.1.

Primary evaluation: exactly one H_U enclosure call on
    hull(I_0 union I_A0) x {lambda_start}.
...
F_JOINT_C1_LEMMA = "CERTIFICATES/prolate/item2_circle/b_tube_v2_1/monotone/F_JOINT_C1_LEMMA_V1.md"
...
REL_RUNNER = "CERTIFICATES/prolate/item2_circle/b_tube_v2_1/monotone/cell0_start_join_union_hu_certificate_v2.py"
...
EXPECTED_F_JOINT_C1_JUDGE_SHA256 = "4d9976395d6438d5d0a99b22b312f3b171d50ae65363e46c5b9e491488de120e"
EXPECTED_F_JOINT_C1_LEMMA_SHA256 = "e04f63e5b6b7f050bf227dfc3066ec08dbb34a53c664b98a78a661682ea099e3"
...
def exact_geometry_gate() -> None:
    if not (CELL0_R_LO < A0_R_LO < CELL0_R_HI < A0_R_HI < 1):
        fail("START_JOIN_GEOMETRY_ORDER")
    if HULL_R_LO != CELL0_R_LO:
        fail("HULL_LO_IDENTITY")
    if HULL_R_HI != A0_R_HI:
        fail("HULL_HI_IDENTITY")
    if max(CELL0_R_LO, A0_R_LO) > min(CELL0_R_HI, A0_R_HI):
        fail("CELL0_A0_DISJOINT")
    if min(CELL0_R_LO, A0_R_LO) != HULL_R_LO:
        fail("CELL0_A0_UNION_LO")
    if max(CELL0_R_HI, A0_R_HI) != HULL_R_HI:
        fail("CELL0_A0_UNION_HI")
...
def main() -> int:
    ...
    if sha256_file(repo / F_JOINT_C1_JUDGE) != EXPECTED_F_JOINT_C1_JUDGE_SHA256:
        fail("F_JOINT_C1_JUDGE_SHA")
    ...
    if joint.get("evidence_class_after_approval") != "HUMAN_AUDITED":
        fail("F_JOINT_C1_EVIDENCE_CLASS")
    if joint.get("lemma_path") != F_JOINT_C1_LEMMA:
        fail("F_JOINT_C1_LEMMA_PATH")
    if joint.get("lemma_sha256") != EXPECTED_F_JOINT_C1_LEMMA_SHA256:
        fail("F_JOINT_C1_LEMMA_PIN")
    if sha256_file(repo / F_JOINT_C1_LEMMA) != EXPECTED_F_JOINT_C1_LEMMA_SHA256:
        fail("F_JOINT_C1_LEMMA_SHA")
    ...
    evaluated: list[dict] = []
    terminal: list[dict] = []
    unresolved: list[tuple[Fraction, Fraction, str]] = [(HULL_R_LO, HULL_R_HI, "S0_BASE/r0")]
    ...
    if unresolved:
        ...
        # Exact terminal closed cover of the start-join union hull; interiors may not overlap.
        ordered = sorted(terminal, key=lambda x: Fraction(x["r_lo"]))
        if not ordered or Fraction(ordered[0]["r_lo"]) != HULL_R_LO or Fraction(ordered[-1]["r_hi"]) != HULL_R_HI:
            fail("UNION_COVER_ENDPOINTS")
        for a, b in zip(ordered, ordered[1:]):
            if Fraction(a["r_hi"]) != Fraction(b["r_lo"]):
                fail("UNION_COVER_GAP_OR_OVERLAP")
        if any(Fraction(x["lo"]) <= 0 for x in terminal):
            fail("TERMINAL_NONPOSITIVE_LO")
        verdict = "START_JOIN_UNION_MONOTONICITY_PASS"
        union_pass = True
    ...
    receipt = {
        "schema": "monotone-tube-v1.1-cell0-start-join-union-hu-v2",
        ...
        "f_joint_c1_judge_signature_sha256": EXPECTED_F_JOINT_C1_JUDGE_SHA256,
        "f_joint_c1_lemma_sha256": EXPECTED_F_JOINT_C1_LEMMA_SHA256,
        ...
        "geometry": {
            "cell0": {"r_lo": fstr(CELL0_R_LO), "r_hi": fstr(CELL0_R_HI)},
            "a0": {"r_lo": fstr(A0_R_LO), "r_hi": fstr(A0_R_HI)},
            "hull": {"r_lo": fstr(HULL_R_LO), "r_hi": fstr(HULL_R_HI)},
            "lambda_start": fstr(LAMBDA_START),
            "lambda_width": "0/1",
            "cell0_a0_overlap": {
                "r_lo": fstr(A0_R_LO),
                "r_hi": fstr(CELL0_R_HI),
            },
        },
        "quantity": "H_U",
        "required_sign": "POS",
        ...
        "union_lo": None if margin is None else fstr(margin),
        "all_terminal_lo_positive": union_pass,
        "union_equals_parent": union_pass,
        "start_join_union_monotonicity": "PASS" if union_pass else "UNRESOLVED",
