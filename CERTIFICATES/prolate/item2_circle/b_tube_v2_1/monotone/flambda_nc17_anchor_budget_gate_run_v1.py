#!/usr/bin/env python3
"""Dedicated deterministic NC17 F_lambda budget gate runner.

This is F_LAMBDA_CONTRACT_V1.1 preexecution infrastructure.

Scope:
- execute the V1.4 gate-unit harness;
- require the real-checker NC17 direct gate to pass;
...
EXPECTED_CODE = "FAIL_ANCHOR_BUDGET"
VERDICT = "NC17_EXACT_SUBCODE_PASS_NOT_PROMOTED"

REQUIRED_STDOUT_LINES = (
    "NC17_DIRECT_GATE=PASS",
...
def main() -> int:
    ...
    if head_post != head_pre:
        stop("HEAD changed during NC17 gate")
    if git("status", "--porcelain"):
        stop("SOURCE_TREE_POST dirty")

    receipt = {
        "schema": "flambda-nc17-anchor-budget-gate-run-v1",
        "contract": "F_LAMBDA_CONTRACT_V1.1",
        "control_id": "NC17",
        "method": "SYNTHETIC_PREDICATE_REAL_CHECKER_FUNCTION",
    ...
    print("CONTROL_ID=NC17")
    print("METHOD=SYNTHETIC_PREDICATE_REAL_CHECKER_FUNCTION")
