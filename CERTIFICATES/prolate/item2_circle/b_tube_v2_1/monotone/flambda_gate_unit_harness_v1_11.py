...
# V1.10 / NC08 and NC09:
# Exercise the exact real-checker anchor-side predicate.  The production
# derivation fixes NEG to LEFT and POS to RIGHT, so these unit controls
# isolate the implementation predicate with one canonical side mutation at
# a time.  They do not claim execution of the canonical producer/receipt
# mutation and do not invoke anchor evaluation or numerical kernels.
#
# NC08: R_HI / NEG incorrectly uses RIGHT; R_LO / POS remains RIGHT.
_nc08_hi_contract = {"anchor_side": "RIGHT"}
_nc08_lo_contract = {"anchor_side": "RIGHT"}

expect_checker_failure(
    lambda: checker._need(
        _nc08_hi_contract["anchor_side"] == "LEFT"
        and _nc08_lo_contract["anchor_side"] == "RIGHT",
        "FAIL_ANCHOR_SIDE_DERIVATION",
    ),
    "FAIL_ANCHOR_SIDE_DERIVATION",
)

print("NC08_DIRECT_GATE=PASS")

# NC09: R_HI / NEG remains LEFT; R_LO / POS incorrectly uses LEFT.
_nc09_hi_contract = {"anchor_side": "LEFT"}
_nc09_lo_contract = {"anchor_side": "LEFT"}

expect_checker_failure(
    lambda: checker._need(
        _nc09_hi_contract["anchor_side"] == "LEFT"
        and _nc09_lo_contract["anchor_side"] == "RIGHT",
        "FAIL_ANCHOR_SIDE_DERIVATION",
    ),
    "FAIL_ANCHOR_SIDE_DERIVATION",
)

print("NC09_DIRECT_GATE=PASS")

# V1.9 / NC02a and NC02b:
# Exercise the exact real-checker runtime formula-ID gates in
# _verify_transport_gate.  NC02a deliberately mismatches only the ordinary
# formula ID.  NC02b satisfies the preceding ordinary gate and deliberately
# mismatches only the Duffy formula ID.  Both stop before transport receipt,
# signature, policy, numerical evaluator, or kernel processing.
class NC02aFK:
    ORDINARY_FORMULA_ID = "NC02A_RUNTIME_ORDINARY"
    DUFFY_FORMULA_ID = "NC02_SHARED_DUFFY"
...
expect_checker_failure(
    lambda: checker._verify_transport_gate(
        pre={
            "actual": {},
            "native_manifest": {
                "ordinary_formula_id": "NC02A_MANIFEST_ORDINARY_MISMATCH",
                "duffy_formula_id": NC02aRoute.fk.DUFFY_FORMULA_ID,
            },
        },
        route=NC02aRoute,
        model=None,
...
print("NC02a_DIRECT_GATE=PASS")


class NC02bFK:
    ORDINARY_FORMULA_ID = "NC02_SHARED_ORDINARY"
    DUFFY_FORMULA_ID = "NC02B_RUNTIME_DUFFY"
...
expect_checker_failure(
    lambda: checker._verify_transport_gate(
        pre={
            "actual": {},
            "native_manifest": {
                "ordinary_formula_id": NC02bRoute.fk.ORDINARY_FORMULA_ID,
                "duffy_formula_id": "NC02B_MANIFEST_DUFFY_MISMATCH",
            },
        },
        route=NC02bRoute,
        model=None,
...
print("NC02b_DIRECT_GATE=PASS")

# V1.8 / NC27:
# Exercise the exact checker predicate used by _precheck for the shared
# F_lambda kernel manifest SHA pin.  The historical _precheck wrapper also
# enforces source-baseline state that intentionally does not match this later
# gate-development HEAD, so this unit isolates the NC27 predicate itself.
# This does not claim execution of the canonical tracked SOURCE_MANIFEST
# mutation.  No tracked file, numerical evaluator, or kernel is touched.
_nc27_actual_shared_kernel_sha256 = "a" * 64
_nc27_manifest_shared_kernel_sha256 = "b" * 64

expect_checker_failure(
    lambda: checker._need(
        _nc27_manifest_shared_kernel_sha256
        == _nc27_actual_shared_kernel_sha256,
        "FAIL_KERNEL_MANIFEST_PIN",
    ),
    "FAIL_KERNEL_MANIFEST_PIN",
)

print("NC27_DIRECT_GATE=PASS")

# V1.7 / NC14:
# Exercise the exact real-checker _verify_transport_gate receipt-SHA pin.
# All three preceding runtime identity gates are satisfied.  Only the pinned
# transport receipt SHA is deliberately mismatched.  The checker must stop at
# FAIL_TRANSPORT_RECEIPT_PIN before any later signature/policy gate.  No
# numerical evaluator or kernel is called.
class NC14FK:
    ORDINARY_FORMULA_ID = "NC14_ORDINARY_FORMULA"
    DUFFY_FORMULA_ID = "NC14_DUFFY_FORMULA"
...
_nc14_original_load_json = checker._load_json

try:
    def _nc14_load_json(path):
        if path == checker.TRANSPORT_PIN_FILE:
            return {
                "receipt_sha256": "PINNED_RECEIPT_SHA",
            }
        return _nc14_original_load_json(path)

    checker._load_json = _nc14_load_json

    expect_checker_failure(
        lambda: checker._verify_transport_gate(
            pre={
                "actual": {
                    "transport_receipt_sha256": "ACTUAL_RECEIPT_SHA",
                    "judge_signature_sha256": "UNREACHED",
                },
                "native_manifest": {
                    "ordinary_formula_id": NC14Route.fk.ORDINARY_FORMULA_ID,
                    "duffy_formula_id": NC14Route.fk.DUFFY_FORMULA_ID,
                },
            },
            route=NC14Route,
            model=None,
    ...
    checker._load_json = _nc14_original_load_json

print("NC14_DIRECT_GATE=PASS")

# V1.6 / NC13:
# Exercise the exact real-checker _anchor_check evidence-class predicate with
# a deterministic synthetic evaluator.  No numerical evaluator or kernel is
# called.  The evaluator returns a valid placeholder interval but deliberately
# returns non-dict evidence, so the first anchor evidence gate must emit the
# exact NC13 subcode.
class NC13Interval:
    lo = Fraction(-2)
    hi = Fraction(-1)
    ...
    @staticmethod
    def to_json():
        return {"synthetic": "nc13"}


class NC13Evaluator:
    @staticmethod
    def _evaluate_exact(*args, **kwargs):
        return None, NC13Interval(), "DIAGNOSTIC_NOT_BINDING"


expect_checker_failure(
    lambda: checker._anchor_check(
        evaluator=NC13Evaluator(),
    ...
    "FAIL_ANCHOR_EVIDENCE",
)
print("NC13_DIRECT_GATE=PASS")

# V1.11 / NC17:
# Exercise the exact real-checker _anchor_check budget predicate with
# deterministic synthetic evidence.  This is a gate-unit test only and does
# not claim execution of the canonical NUMERIC_TINY_CAP control or any
# checker-source cap mutation.  The synthetic evaluator performs no numerical
# kernel work.  All evidence fields preceding the budget gate are valid, and
# only boundary_route_evaluation_count_delta exceeds the checker anchor cap.
class NC17Evaluator:
    @staticmethod
    def _evaluate_exact(*args, **kwargs):
        evidence = {
            ...
            "boundary_route_evaluation_count_delta":
                checker.CHECKER_ANCHOR_CALL_CAP + 1,
        }
        # The interval is intentionally unused: FAIL_ANCHOR_BUDGET fires
        # before either anchor-sign predicate is reached.
        return None, None, evidence


expect_checker_failure(
    lambda: checker._anchor_check(
        evaluator=NC17Evaluator(),
        endpoint=checker.Dyadic(0, 0),
        lam=Fraction(1, 2),
        required_sign="NEG",
        config={
