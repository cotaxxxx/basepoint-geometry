...
def certify_node(node: str, candidate_index: int, u_max: Fraction,
    ...
    while pending:
        ...
        body: dict[str, Any] = {
            ...
            "evaluation_count": 1 if did_evaluate else 0,
        ...
        if node == "L1":
            u0, u1, s0, s1 = domain
            body.update(
                u_interval=interval_json(u0, u1),
                s_interval=interval_json(s0, s1),
                global_domain={
                    "u_interval": interval_json(Fraction(0), u_max),
                    "s_interval": interval_json(-S_NEG, s_start),
                },
                quantity="H_u_EQUALS_NEGATIVE_F_r",
            )
        else:
            s0, s1 = domain
            face = u_max if node == "L2" else Fraction(0)
            body.update(
                u_face=dyadic_json(face),
                s_interval=interval_json(s0, s1),
                global_domain={
                    "u_face": dyadic_json(face),
                    "s_interval": interval_json((-S_NEG if node == "L2" else Fraction(0)), s_start),
                },
