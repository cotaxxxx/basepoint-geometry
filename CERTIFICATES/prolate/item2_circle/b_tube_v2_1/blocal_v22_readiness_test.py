...
def load(config:dict[str,Any])->tuple[Any,Any,Any,Any]:
    ...
    l3=provenance.load_pinned_module(ROOT,{"path":lp["path"],"sha256":lp["source_sha256"]},"blocal_v22_readiness_l3_bprime",("prepare","certify_l3"),{"ROUTE_ID":model.L3_BPRIME_ROUTE_ID,"POLICY_ID":model.L3_BPRIME_POLICY_ID})
    return route,adapter,kernel,l3
...
def main(argv:list[str]|None=None)->int:
    ...
    ctx.prec=config["precision"]["bits"];route,adapter,kernel,l3=load(config);helper=route.validate_helper_lemmas(arb,fmpq,config);l3p=l3.prepare(ROOT,config)
    ...
    l3rec,l3ok,l3why,l3calls=l3.certify_l3(l3p,adapter,0,lam,s_first,config)
    model.need(l3ok and l3why is None and l3calls==1,"readiness L3 Bprime monotonicity")
    payload={"schema":"blocal-v22-readiness-v3","certificate_evidence":False,"eps":config["geometry"]["eps"],
             "helper_lemmas":helper,"initial_F":f0,"midpoint_F":fm,"positive_width_derivative_Hu":hu,"F_r":D,"quotient":q,"newton_image":newton,
             "corner_strip_Hu":hcorner,"l3_bprime_monotonicity":l3rec,"proof_ids":{"initial_F":pf0["proof_id"],"midpoint_F":pfm["proof_id"],"derivative":phu["proof_id"],"corner":pcorner["proof_id"]},
