...
def main()->int:
    ...
    paths={"policy":repo/REL_POLICY,"replay_producer":repo/REL_REPLAY_PRODUCER,
           "production_producer":repo/REL_PRODUCTION_PRODUCER,"frozen_checker":repo/REL_FROZEN_CHECKER,
           "contract":repo/REL_CONTRACT,"core":repo/REL_CORE,"geometry_module":repo/REL_GEOMETRY,
           "receipt":receipt_path,"attestation":att_path,"tube_geometry":geom_path}
    ...
    checks={"policy":POLICY_SHA256,"replay_producer":REPLAY_PRODUCER_SHA256,
            "production_producer":PRODUCTION_PRODUCER_SHA256,"frozen_checker":FROZEN_POSITIVE_CONTROL_CHECKER_SHA256,
            "core":CORE_SHA256,"geometry_module":GEOMETRY_MODULE_SHA256}
    ...
    receipt=json.loads(receipt_path.read_text()); att=json.loads(att_path.read_text()); geometry=json.loads(geom_path.read_text())
    raw_sha=sha(receipt_path)
    ...
    if observed_producer==REPLAY_PRODUCER_SHA256:
        ...
        if att.get("positive_control_receipt_reused") is not False or att.get("fresh_reexecution") is not True:
            fail("REPLAY_ATTESTATION_PROVENANCE")
        if att.get("released_producer_sha256")!=REPLAY_PRODUCER_SHA256:
            fail("REPLAY_ATTESTATION_PRODUCER_PIN")
        if att.get("released_checker_sha256")!=FROZEN_POSITIVE_CONTROL_CHECKER_SHA256:
            fail("REPLAY_ATTESTATION_FROZEN_CHECKER_PIN")
        ...
        if att.get("production_producer_sha256")!=PRODUCTION_PRODUCER_SHA256:
            fail("PRODUCTION_ATTESTATION_PRODUCER_PIN")
    else:
        fail("RECEIPT_PRODUCER_NOT_ALLOWLISTED")
    ...
    try:
        derived=derive_parent(geometry)
    ...
    c=geometry.get("candidate_inputs",{})
    ...
    try:
        ...
        fail(str(exc))
    print("CHECKER_ID=PRODUCTION_HU_DOMAIN_V1_2_PRODUCTION_CHECKER_V2")
    print("EVIDENCE_CLASS=PRODUCTION_CANDIDATE")
    print("PRODUCER_ROLE="+role)
    print("PRODUCER_SHA256="+observed_producer)
    print("PARENT_SOURCE=MONOTONE_TUBE_V1_1_COMPONENT1_RECONSTRUCTED")
    print("RECEIPT_PARENT_EXACT_MATCH=TRUE")
    print("CELL0_POLICY_PARENT_ROLE=POSITIVE_CONTROL_CROSSCHECK_ONLY")
